...
|---|---|---|
| **V8 port breakage windows** | V8 PPC64 is an **externally-maintained port** (IBM + Red Hat via `PPC_OWNERS`) — stable since ~2014, not slated for removal. Verified alive on 149.x: `v8/src/codegen/ppc/` (16 kLoC), `v8/src/maglev/ppc/` (Maglev ported), `V8 Linux - ppc64 - sim` CI builder. Google CI doesn't gate on PPC so V8 can land breakage for days before the port team catches up. Our three V8 patches (POWER8 baseline, pointer compression, trap asm) are tuning, not replacement. **Precedent:** MIPS(64) was actually removed from V8 in 7.5 (2019) because maintenance lapsed — different story from PPC, easy to confuse. | After each Chromium roll, `git diff main..ppc64le -- v8/` should be small. If V8 builds break inside ppc codegen, look for an upstream port commit on `chromium-review.googlesource.com` by IBM/RH and cherry-pick. |
